...
def check(written, words_text):
    ...
    for i in range(len(written)):
        if i < len(words_text) and written[i] == words_text[i]:
            count += 1
    acc = (count * 100) // len(written) if written else 0
    return acc
...
def check_input(window, canvas1, write, words_text):
    ...
    written = write.get("1.0", "end-1c").strip().split()
    wpm = len(written)
    ...
    try_again = Button(text="Try Again", command=partial(again, window), font=("Helvetica", 13, "bold italic"),
                       bg="#c80815", fg="white", relief="raised")
...
def start(mins, sec, window, canvas1, write, words_text):
    ...
    window.after(1000 * temp, lambda: check_input(window, canvas1, write, words_text))


def choose_text():
    try:
        with open("sentences.txt", "r") as fw:
            sen = fw.readlines()
        return random.choice(sen).strip()
    except FileNotFoundError:
        messagebox.showerror("Error", "The sentences file is missing.")
        return "Please create a sentences.txt file."
